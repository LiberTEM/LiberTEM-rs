...
use clap::Parser;
use ipc_test::{SHMInfo, SharedSlabAllocator, SlotInfo};
...
fn main() {
    ...
    let mut stream = UnixStream::connect(args.socket_path).expect("connect to socket");
    ...
    let shm_info: SHMInfo = bincode::deserialize(&bytes[..]).expect("deserialize SHMInfo object");

    let mut ssa = SharedSlabAllocator::connect(fds[0], &shm_info).unwrap();
